...
document.addEventListener('DOMContentLoaded', () => {
    carregarDados('http://localhost:3000/profissionais', processarDadosProfissional);
    carregarDados('http://localhost:3000/clientes', processarDadosCliente);
});

async function carregarDados(url, processarDados) {
    try {
...
// Avaliacao Prestador de Servico
function processarDadosProfissional(dados) {
    const profissional = dados[0];
    document.getElementById('imagemProfissional').src = profissional.fotoUrl;
    document.getElementById('nomeProfissional').textContent = profissional.nome;
    document.getElementById('descricaoServico').textContent = profissional.descricao;
    const avaliacaoProfissionalSection = document.getElementById('avaliacaoProfissionalSection');
    for (let i = 1; i <= 5; i++) {
        const starIcon = document.createElement('span');
        starIcon.classList.add('star-icon');
        starIcon.dataset.value = i;
        starIcon.addEventListener('click', function (event) {
            marcarEstrelasProfissional(profissional, event.target.dataset.value);
        });
        avaliacaoProfissionalSection.appendChild(starIcon);
    }
}

function marcarEstrelasProfissional(profissional, valor) {
    const stars = document.querySelectorAll('.star-icon');
    stars.forEach(star => {
        const starValue = parseInt(star.dataset.value);
        const isStarSelected = starValue <= valor;
        star.classList.toggle('ativo', isStarSelected);
    });
    enviarAvaliacaoP(profissional, valor);
}

async function enviarAvaliacaoP(profissional, valor) {
    const profissionalId = profissional.id;
    try {
        const response = await fetch(`http://localhost:3000/profissionais/${profissionalId}`, {
            method: 'PATCH',
            headers: {
...
function processarDadosCliente(dados) {
    if (dados.length === 0) {
        console.error('Nenhum dado encontrado.');
        return;
    }
    const cliente = dados[0];
    document.getElementById('imagemCliente').src = cliente.fotoUrl;
    document.getElementById('nomeCliente').textContent = cliente.nome;
    document.getElementById('descricaoServicoSolicitado').textContent = cliente.descricaoServicoSolicitado;

    const avaliacaoClienteSection = document.getElementById('avaliacaoClienteSection');
    for (let i = 1; i <= 5; i++) {
        ...
        starCIcon.addEventListener('click', function (event) {
            marcarEstrelasCliente(cliente, event.target.dataset.value);
        });
        avaliacaoClienteSection.appendChild(starCIcon);
    }
}

function marcarEstrelasCliente(cliente, valor) {
    ...
    enviarAvaliacaoCliente(cliente, valor);
}

async function enviarAvaliacaoCliente(cliente, valor) {
...
function perfilCliente(){
    window.location.href = "lista perfil.html";
}
//termos de uso
